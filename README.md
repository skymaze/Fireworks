...
- **模型 / 镜像**：控制平面只下载一份；经管理网发送 head 后，由 worker Agent 通过规划的高速 IP 并行直拉，不依赖节点间 SSH/rsync，并展示逐节点进度、速度与当前文件。模型支持多版本共存、增量更新、发布版本钉扎与历史版本回滚/GC，并把当前版本平铺为可直接 `cp -r` 拷贝的真实文件目录（硬链接，几乎不占额外磁盘）；节点侧镜像可在节点详情直接查看与删除
...
没有域名和 HTTPS 反向代理时使用。命令会拉取 `latest` 标签镜像并启动服务，随后通过 `http://部署主机IP:3000` 访问。`FW_IMAGE_TAG` 默认即为 `latest`（每次拉最新发布），需要钉扎版本时可显式指定，如 `FW_IMAGE_TAG=0.15.0`。
...
6. **发布任务**：`任务 → 发布任务`：选配方 → 选集群 → 指定 head/worker 与各节点 rank（head 固定 rank 0）→ 配置变量 → 预览 → 发布。预览和发布都会先从所选 Agent 刷新节点信息，避免用旧硬件参数渲染；发布前会保障模型与镜像就绪，两类都缺失时同时启动各自的传输（可并发），全部就绪后才会创建任务
...
| `fireworks-cache` | `/data/cache` | 控制平面模型缓存、模型平铺镜像、镜像归档、配方源镜像 | 大容量 SSD / HDD，可重新获取 |
...
升级到当前版本（0.15.0）：可直接复用已有 `fireworks-db`，无新增数据迁移；本次节点 Agent 有行为变更（回拉错误映射、模型平铺镜像、删除模型时清理镜像目录），升级控制面后需重新部署节点 Agent。完整升级说明见 [v0.15.0 发布说明](docs/releases/v0.15.0.md)，历史版本见 [docs/releases/](docs/releases/)。
...
├── docs/model-transfer.md     # 模型 manifest、Agent 高速直传、平铺镜像、进度与恢复
