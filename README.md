...
3. **创建集群**：`集群 → 创建集群`，连续勾选成员节点时不会自动发起耗时网络请求；可按需点击「网络预检」提前查看结果，也可直接创建。提交后只执行一次权威检查，节点快照读取、ARP 探测、网络应用和验证均按节点并行；一致配置直接复用，混合网段推荐空闲网段并统一重配。**全部验证通过才创建、失败自动回滚**，完成后从 Agent 刷新节点信息，避免数据库保留旧网络地址
