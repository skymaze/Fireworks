        ...
        <UContainer>
          <div class="flex items-center justify-between h-16">
            ...
            <div class="flex items-center gap-1 min-w-0">
              <UPopover v-model:open="mobileNavOpen">
                <UButton
                  class="lg:hidden"
                  variant="ghost"
                  color="neutral"
                ...
                <template #content>
                  <div class="w-56 p-1">
                    <NuxtLink
                      ...
                      :key="item.to"
                      :to="item.to"
                      class="block px-3 py-2 rounded-md text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"
              ...
              <NuxtLink to="/" class="flex items-center gap-2 text-lg font-bold min-w-0">
                <span>🎆</span>
                <span class="truncate">Fireworks</span>
                <span class="hidden md:inline text-xs font-normal text-gray-400 dark:text-gray-500">{{ t('nav.subtitle') }}</span>
              </NuxtLink>
            </div>
            <!-- 桌面端导航（lg+ 显示） -->
            ...
            <div class="flex items-center gap-1">
              ...
              <UPopover v-model:open="userMenuOpen">
                <UButton
                  ...
                  class="max-w-[10rem]"
                >
                  <span class="hidden sm:inline-block truncate">{{ auth.state.value.username || '-' }}</span>
                </UButton>
                <template #content>
                  <div class="w-40 p-1">
                    <button
