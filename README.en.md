...
- **Tasks**: run containerized workloads with Docker Compose; assign head/worker roles and ranks per node for every deployment, allowing multiple tasks with different role layouts on the same cluster; publish, pause, resume, stop (keeps containers), start, restart, delete, inspect logs, and run health checks — stopped tasks can be **started** again and running tasks **restarted** in place, reusing containers without recreation
