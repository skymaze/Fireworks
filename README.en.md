...
3. **Create a cluster**: go to `Clusters → Create Cluster`. Selecting members does not trigger expensive network requests. Run **Network Preflight** when you want to inspect results early, or create the cluster directly. Submission performs one authoritative check: node snapshots, ARP discovery, network application, and validation all run in parallel by node. Consistent existing configurations are reused; mixed subnets result in a recommended free subnet and uniform reconfiguration. The cluster is created only if all checks pass, and failures roll back automatically. Node data is refreshed from the Agents afterward so stale network addresses are not retained.
...
7. **Operate tasks and assets**: inspect logs; pause, resume, stop, or delete tasks; run health checks; and track model and image distribution. A single real-time stream handles both log history and live updates. Deleting a task also removes its inference and benchmark data; see [Task lifecycle](docs/task-lifecycle.md). Asset acquisition selects the first node and the remaining nodes by default, then performs download, distribution, and verification automatically. [Model transfer](docs/model-transfer.md) and [Image transfer](docs/image-transfer.md) describe their state machines and recovery behavior.
