...
镜像管理通过 WebUI 完成，不要求用户登录节点、配置节点间 SSH 互信或手工运行脚本。输入镜像后一次点击立即拉取到控制平面；拉取完成后，在镜像归档列表点击“分发”，再在弹窗中先选集群、后选节点。节点默认全选，第一个选中的节点作为 head，其余节点作为 worker。后端同时拒绝未加入集群或跨集群的分发节点。
