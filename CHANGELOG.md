...
- **节点磁盘写满不再是看不懂的失败**：
  - Agent 部署（重装/升级）在上传前检查目标分区剩余空间，不足时直接提示「节点磁盘空间不足：剩余 X，至少需要 Y（含离线依赖与 venv 创建）」并给出清理建议。此前磁盘写满时 SFTP 上传只返回不带 errno 的 `Failure`，界面看不出原因；旧 Agent 因文件没换成功继续运行，表现为「部署失败但版本没变」。
  - 模型文件写入时磁盘写满（ENOSPC）现在返回 507「节点磁盘空间不足，无法写入模型文件」，不再归类为「控制平面回拉失败(网络/IO)」而误导排查方向。

### Upgrade

- 控制面与前端升级到 `0.15.2`；本次节点 Agent 有行为变更（磁盘满错误分类），需重新部署节点 Agent（与 0.15.0 的 Agent 行为变更一次重部署同时生效）。
- 无新增数据迁移。
- 提示：节点磁盘写满通常来自历史镜像与模型缓存。部署前可用 `df -h /` 确认剩余空间，清理不再使用的 Docker 镜像与模型版本后再重试。

## [0.15.1] - 2026-09-10

### Changed

- **传输并发放开：只禁止同一模型/镜像到同一台机器的重复传输**：此前控制平面同一时间只允许一个外部下载源（模型下载与镜像拉取互斥，v0.14.0 引入），且同名资源只要存在进行中任务就不再接受新任务——即使目标机器完全不同。现在模型与镜像、多个模型、多个镜像都可以同时传输，控制平面下载/拉取与节点分发互不阻塞；唯一保留的限制是同一份资源的重复操作：同一模型/镜像 -> 相同机器（head 或任一 worker）的重复分发会被拒绝（同机同文件的并发写），同一模型/镜像的重复真实下载/拉取也会被拒绝（同写控制平面缓存/归档）。同一模型分发到不相交的节点集合（如分发到不同集群）现在可以与其它传输同时进行。
- **发布一次启动全部传输**：发布任务时模型与镜像分别保障，任一未就绪不再跳过其余资源，一次发布会启动所有缺失的模型/镜像传输（该行为随 0.15.0 生效，本次补齐并发准入使其真正可以同时进行）。

### Fixed

- **同机重复分发提示更准确**：同一模型/镜像向相交节点集合重复分发时，提示「正在向相同节点分发（任务 #N）」并给出任务号，不再笼统提示「已有进行中的传输任务」——目标机器不相交的其它任务不受影响。

### Upgrade

- 控制面与前端升级到 `0.15.1`；本次节点 Agent 无变更，无需重部署（仍在 0.15.0 之前、尚未重部署 Agent 的环境先按 0.15.0 说明重部署一次）。
- 无新增数据迁移。
...
- **模型平铺镜像可直接拷贝**：平台缓存采用 Hugging Face hub 布局（`blobs/` + `snapshots/<sha>/` 符号链接），直接 `cp -r` 只会得到指向 blobs 的符号链接而不是模型文件。现在控制平面与各节点在下载/分发完成后，会把当前激活版本平铺成真实文件目录（控制平面 `MODEL_FILES_DIR`，默认 `/data/cache/models-files`；节点 `~/.cache/huggingface/models`）：同文件系统用硬链接，几乎不占额外磁盘；跨文件系统自动回退为复制；只保留当前激活版本，切换版本后清理旧文件，删除模型时镜像目录一并删除。已有的完整缓存无需重新下载——打开本地模型列表即自动补生成。
- **发布任务同时分发模型与镜像**：此前发布时模型保障未就绪就返回 409，镜像保障根本不会执行，一次发布只能启动一类传输。现在一次发布同时启动所有缺失资源的传输（两类传输可并发；仅「模型下载 + 镜像拉取」这类真实外部下载仍按既有策略互斥），并在同一条提示里汇报各类传输任务号；前端发布按钮不再因资源未就绪而禁用，未就绪时发布会自动启动传输，也可继续用「发送模型 / 发送镜像」按钮查看进度。

### Fixed

- **模型/镜像回拉失败不再显示裸 500**：head 节点 Agent 从控制平面回拉模型文件（`/api/model/pull`）或镜像归档失败时，异常此前直接冒出为 `500 Internal Server Error`，控制平面任务错误里只有 httpx 原文 `Server error '500 Internal Server Error' for url ...`，看不出真实原因。现在 Agent 区分返回 502（控制平面明确 4xx/5xx，携带状态码、原因与响应摘要）或 504（断流/超时/IO），控制平面从结构化 detail 中提取文案写入任务错误，直接显示真实原因。
- **回拉 URL 编码**：控制平面下发给 Agent 的模型文件回拉 URL 现在对 repo / relpath 做 URL 编码——文件名含 `&`、`#`、`+`、空格、`%` 等字符时，此前 query 会被截断/污染，控制平面按错误路径查不到文件返回 404，节点再以 500 上报，表现为任务无提示地失败。普通文件名的 URL 保持不变。
...
- **允许主机只接一条高速网线建集群**：此前创建/扩容集群的物理链路预检要求每台节点 4 个高速口全部有载波，只接一条线时会被「高速网络物理链路预检失败」拦下。现在未接线的口只提示告警、不再阻止：每台节点只需至少一条高速链路可用（完全没有物理链路的节点仍会被拦截），配置后的连通验证也只对已接线的口生效，并要求与每个对端至少有一条 rail 互通——把「插了线就必须通」的严格性保留在真正接线的链路上，而不是约束未插线的口。
- **发布任务自动填充跟随已接线的口**：任务自动变量（`node_roce_ip`、`NCCL_SOCKET_IFNAME`、`NCCL_IB_HCA`、`NCCL_IB_GID_INDEX`，分布式 `MASTER_ADDR` 跟随 head 的 `node_roce_ip`）此前以「存在 RoCEv2 GID/IP 即视为链路可用」选择主口——单线场景下未接线的口也可能有静态 IP 与派生 GID，导致主口选到没接线的 rail、任务分布式初始化失败。现在按 IB 端口物理状态（ACTIVE/ARMED）优先选已接线的口：只接一条线时 `MASTER_ADDR`/`VLLM_HOST_IP` 一定会落在能互通的 rail 上，`NCCL_IB_HCA` 只列已接线的 HCA；四条线全接时行为不变。

## [0.14.3] - 2026-09-03

### Fixed

- **首页推理图表悬空柱**：首页「Decode 吞吐」与「输入 token 量」堆叠柱状图在时间轴上出现悬空柱——ECharts 当前在时间轴上做堆叠时按系列数据索引对齐基线，多个任务有数据的时间桶不一致时，后绘制的系列会把前一任务**不同时刻**的值当作堆叠起点，柱子底部离开基线。现在各任务系列先按全局时间桶对齐（缺失桶补空），再交给 ECharts 堆叠，柱子始终从基线起量，空白时段保持无柱。
